...
Для этапов обучения (`classifier_training`, `metric_classifier_training`) при необходимости можно сразу передать готовую валидационную выборку через `data.valid_parquet_path`. В этом случае датасет будет дополнительно оцениваться на отдельном файле, а `val_ratio` продолжит задавать долю сплита внутри обучающей выборки.
