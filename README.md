...
Для этапов обучения (`classifier_training`, `metric_classifier_training`) требуется сразу передать две выборки: `data.parquet_path` для обучения и `data.valid_parquet_path` для валидации. Внутренний сплит больше не создаётся — метрики считаются только на переданном валидационном файле.
