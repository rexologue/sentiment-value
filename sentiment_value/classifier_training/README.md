...
- `data.parquet_path` — путь к обучающему Parquet-датасету; `valid_parquet_path` — обязательный файл валидации; `num_workers` — воркеры даталоадеров; `upsample`/`downsample` — балансировка классов.
...
Валидация теперь проводится только на `valid_parquet_path`; внутренний сплит больше не создаётся.
