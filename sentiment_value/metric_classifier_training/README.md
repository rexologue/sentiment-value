...
- `data` — `parquet_path`, обязательный `valid_parquet_path` для валидации, `num_workers`, `upsample`/`downsample`.
...
Метрики считаются только на явной валидационной выборке из `valid_parquet_path`; автоматический сплит по `val_ratio` убран.
